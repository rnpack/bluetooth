...
If you want to use Android Studio or Xcode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/RNPackBluetoothExample.xcworkspace` in Xcode and find the source files at `Pods > Development Pods > @rnpack/bluetooth`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `@rnpack/bluetooth` under `Android`.
...
```sh
Running "RNPackBluetoothExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
